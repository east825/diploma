...
так как пытаются вывести общие типы (например, через типы-объединения) для результата и аргументов. В качестве примера
приводится метод `a.max(b)`, который может принимать, например, аргументы типа `{int,
float}` (как и в Python здесь объект, на котором происходит вызов, считается за первый аргумент),
и для возвращаемого значения которого выводится тип `{int|float}`. Алгоритмы, которые анализруют функцию лишь раз,
авторы называют *моновариантными (monovariant)*, а те, которые создают для функции несколько
шаблонов - *поливариантными (polyvariant)*.
...
Естественно, что такая стратегия ведет к экспоненциальному росту числа шаблонов,
и для любого p можно найти последовательность поливариантных вызовов, для которой алгоритм будет таким же неточным
как и базовый (примеры в статье).

2. Также строятся шаблоны для каждого вызова, но при этом анализируются, для каких вызовов эти шаблоны можно сделать
